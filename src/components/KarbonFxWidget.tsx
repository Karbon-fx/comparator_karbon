...
import { Info, RefreshCw, Plus, Minus } from 'lucide-react';
import { cn } from '@/lib/utils';
import { calculateMarkupPerUsd, calculateSavings, calculateTotalInr, convertFromLogScale, convertToLogScale, formatNumber, formatRate } from '@/lib/utils';
...
const AnimatedNumber = ({ 
  ...
  const rounded = useTransform(motionValue, (latest) => 
    `${prefix}${latest.toFixed(decimals).replace(/\B(?=(\d{3})+(?!\d))/g, ',')}`
  );
...
const CompetitorCard = ({ name, icon, rate, liveRate, usdAmount, karbonTotal, onRateChange, delay, isBestAlternative }: CompetitorCardProps) => {
    ...
    return (
        <motion.div
            ...
            className={cn(
                ...
                isBestAlternative ? "border-karbon-cornflower ring-2 ring-karbon-cornflower ring-offset-2" : "border-gray-200"
            )}
        >
            ...
            <div className="space-y-4 text-sm">
                ...
                <div className="flex justify-between pt-4 border-t border-gray-100">
                    ...
                    <span className="font-bold text-karbon-ebony tabular-nums">
...
export const KarbonFxWidget = ({ initialAmount = 1000, compact = false }: KarbonFxWidgetProps) => {
    ...
    const competitorResults = useMemo(() => {
        const bankTotal = calculateTotalInr(usdAmount, parseFloat(bankRate));
        const paypalTotal = calculateTotalInr(usdAmount, parseFloat(paypalRate));

        return [
            { name: 'Bank', total: bankTotal, savings: calculateSavings(karbonTotalInr, bankTotal) },
            { name: 'PayPal', total: paypalTotal, savings: calculateSavings(karbonTotalInr, paypalTotal) }
        ];
    }, [usdAmount, bankRate, paypalRate, karbonTotalInr]);

    const bestAlternative = useMemo(() => {
        return competitorResults.reduce((best, current) => {
    ...
    return (
        <div className="karbon-fx-widget w-full max-w-5xl mx-auto bg-white rounded-3xl shadow-xl border border-gray-200 overflow-hidden">
            ...
            <div className="px-8 py-8 bg-gradient-to-br from-blue-50/50 to-sky-50/50">
                <div className="mb-8">
                    <label className="block text-sm font-semibold text-[#0C1A2B] mb-3">
                        Amount to Convert
                    </label>
                    <div className="relative">
                        <div className="flex items-center bg-white rounded-2xl border-2 border-gray-200 focus-within:border-[#0066CC] transition-all duration-200 px-6 py-4 shadow-sm">
                            <div className="flex items-center gap-3 mr-4">
                                ...
                                <span className="text-2xl font-bold text-[#0C1A2B]">$</span>
                            </div>
                            <CurrencyInput
                                id="usd-input"
                                name="usd-amount"
                                ...
                                className="flex-1 text-4xl font-bold text-[#0C1A2B] bg-transparent border-0 outline-none focus:ring-0 tabular-nums"
                                placeholder="1,000"
                ...
                <div className="flex items-center justify-center gap-2 mt-6 text-sm">
                    <motion.span animate={{ scale: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 2 }} className="h-2 w-2 rounded-full bg-[#0066CC]" />
                    ...
                    <motion.button whileHover={{ rotate: 180 }} whileTap={{ scale: 0.9 }} onClick={fetchLiveRate} disabled={isRefreshing} className="ml-2 p-1 rounded-md hover:bg-white transition-colors disabled:opacity-50">
                        <RefreshCw className={`h-4 w-4 text-[#0066CC] ${isRefreshing ? 'animate-spin' : ''}`} />
                    </motion.button>
                </div>
            </div>
            ...
            <div className="px-8 py-8 bg-gray-50">
                <div className="text-center mb-8">
                    <h3 className="text-2xl font-bold text-[#0C1A2B] mb-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    >
                        ...
                        <div className="space-y-4 text-sm">
                            ...
                            <div className="flex justify-between pt-4 border-t border-white/20">
                                ...
                                <span className="font-bold tabular-nums">${formatNumber(usdAmount)}</span>
                            </div>
                        </div>
                    </motion.div>
                    ...
                    <CompetitorCard
                        ...
                        delay={0.2}
                        isBestAlternative={bestAlternative.name === 'Bank'}
                    />
                    ...
                    <CompetitorCard
                        ...
                        delay={0.3}
                        isBestAlternative={bestAlternative.name === 'PayPal'}
                    />
                </div>
            </div>
        </div>
